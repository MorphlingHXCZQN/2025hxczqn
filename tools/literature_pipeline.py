...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Destination .docx path for the exported plan (default: <output-dir>/serum_tdp43_sepsis_project.docx).",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.word_output is None:
        args.word_output = args.output_dir / "serum_tdp43_sepsis_project.docx"
    else:
    ...
    logging.info("Processed %s articles; plan + DOCX updated.", len(results))
